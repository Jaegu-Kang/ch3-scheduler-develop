...
```
- **Response:**
- Error (400 Bad Request): 내용은 100자 이하로 입력 가능합니다,제목은 필수 입력 값입니다. (입력값 유효성 검사 실패)
- Error (401 Unauthorized): 로그인 후 작성 가능합니다. (인증되지 않은 사용자)
- Success (201 Created): 데이터 생성 성공 (생성된 객체 반환)
...
- **Response:**
- Error (404 Not Found): 없는 일정 입니다. (잘못된 ID로 요청 시)
- Success (200 OK)
...
```
- **Response:**
- Error (400 Bad Request): 제목은 필수 입력 값입니다, 내용은 100자 이하로 입력가능합니다. 
- Error (401 Unauthorized): 로그인 후 작성 가능합니다. (세션 없음)
- Error (403 Forbidden): 권한이 없습니다. (본인이 작성한 데이터가 아님)
- Error (404 Not Found): 없는 일정 입니다. (잘못된 ID로 요청)
- Success (200 OK)
...
- **Request:** Path Variable {id} (api/schedules/1)
- **Response:** 
- Error (403 Forbidden): 권한이 없습니다. (본인이 작성한 일정이 아님)
- Error (404 Not Found): 없는 일정 입니다. (잘못된 ID로 요청)
- Success (204 No Content): 본문 없음
...
```
- **Response:**
- Error (400 Bad Request): 이메일 형식이 올바르지 않습니다, 비밀번호 형식이 올바르지 않습니다.(입력값 유효성 검사 실패)
- Success (201 Created): 데이터 생성 성공
...
```
- **Response:**
- Error (404 Not Found): 존재하지 않는 이메일입니다. (가입되지 않은 이메일)
- Error (401 Unauthorized): 비밀번호가 일치하지 않습니다. (이메일은 맞으나 비밀번호 불일치)
- Success (204 No Content): 로그인 성공
- **Body:** 본문없음
...
- **Response:**
- Error (404 Not Found): 존재하지 않는 유저입니다. (잘못된 ID로 요청)
- Success (200 OK)
...
```
- **Response:**
- Error (400 Bad Request): 이메일 형식이 올바르지 않습니다. 등 (입력값 유효성 검사 실패)
- Error (403 Forbidden): 권한이 없습니다. (본인이 작성한 데이터가 아님)
- Error (404 Not Found): 존재하지 않는 게시물입니다. (잘못된 ID로 요청)
- Success (200 OK )
...
- **Request:** Path Variable {id} (api/users/1)
- **Response:** 
- Error (403 Forbidden): 권한이 없습니다. (본인의 정보가 아님)
- Error (404 Not Found): 존재하지 않는 유저입니다. (잘못된 ID로 요청)
- Success (204 No Content): 본문 없음
...
```
- **Response:**
- Error (400 Bad Request): 내용은 5글자 이하로 입력 가능합니다. (입력값 유효성 검사 실패)
- Error (401 Unauthorized): 로그인 후 작성 가능합니다. (인증되지 않은 사용자)
- Error (404 Not Found): 존재하지 않는 유저입니다. (세션 정보는 있으나 DB에 해당 유저가 없음)
- Error (404 Not Found): 없는 일정 입니다. (DB에 해당 일정이 없음)
- Success (201 Created): 데이터 생성 성공 (생성된 객체 반환)
